...
class cls_catalogo{
	...
	function control($catalogo_x=""){
		...
		$cn->query = "	SELECT $this->cfg_catalogos.*,$this->cfg_plantillas.diagrama 
						...
						LEFT JOIN $this->cfg_plantillas ON $this->cfg_plantillas.plantilla = $this->cfg_catalogos.plantilla
		...
		return $aux.$this->crear_navegador();
	}// end function            
	//===========================================================
	function crear_detalle($q,$linea_x){
		$q_detalle = $this->evaluar_exp($q_detalle);
		$cn = new cls_conexion;
		$cn->query = $q;
		$result = $cn->ejecutar();
